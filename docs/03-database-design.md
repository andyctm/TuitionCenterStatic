...
| `PasswordResetToken`    | Single-use, time-limited password-reset token (FR-AUTH-3) — added during M1 implementation, missing from v1.0 | hashed token, not raw; `usedAt` enforces single-use                                                                                   |
